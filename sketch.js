...
function spawnObstacles() {
  if (World.frameCount%100 === 0) {
    ...
    // if(bunny.collide(obstacle)) {
    //   gameState = "over";
    // }

    // if(obstacle.isTouching(bunny) || obstacle.isTouching(alien)) {
    //   console.log("dead");
    //   gameState = "over";
    // }
...
function draw() {
  ...
  if(gameState === "steps") {
    ...
    if(player === "bunny") {
      ...
      //jump
      if(keyDown("up") && bunny.y >= height-152.5){
        bunny.velocityY = -20 ;
      ...
      if(bunny.collide(invisiGround)) {
        bunny.changeAnimation("bunnyStanding", bunnyStandImg);
      }

      // if(obstacle.isTouching(bunny)) {
      //   gameState = "end";
      // }
      ...
      //jump
      if(keyDown("up") && alien.y >= height-152.5){
        alien.velocityY = -20 ;
